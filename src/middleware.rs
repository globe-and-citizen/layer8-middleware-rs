...
pub fn server_static(req: JsValue, res: JsValue, dir: JsValue, fs: JsValue) -> JsValue {
    ...
    let return_encrypted_image = |res: &JsValue| {
        ...
        js_sys::Reflect::set(&res, &"statusCode".into(), &JsValue::from_f64(200.0))
            .expect("expected res to be a mutable object");
        js_sys::Reflect::set(&res, &"statusMessage".into(), &JsValue::from_str("OK"))
            .expect("expected res to be a mutable object");
        js_sys::Reflect::set(
            &res,
            &"content-type".into(),
        ...
        let end = js_sys::Reflect::get(&res, &JsValue::from_str("end"))
            .expect("expected res to have an end method");
        let end = js_sys::Function::from(end);
    ...
    let (mp_jwt, symmetric_key) = INMEM_STORAGE_INSTANCE.with(|val| {
        ...
        let client_uuid = client_uuid
            .to_string()
            .expect("expected client_uuid to be a string; qed");

        (
            val_.jwts.get(&client_uuid).cloned(),
    ...
    let add_event_listener = js_sys::Function::from(
        js_sys::Reflect::get(&req, &JsValue::from_str("addEventListener"))
            .expect("expected req to have an addEventListener method"),
    );
    ...
    let on_end: &Closure<dyn FnMut(wasm_bindgen::JsValue) -> JsValue> =
        &Closure::new(move |_arg| {
            ...
            js_sys::Reflect::set(&req, &"method".into(), &JsValue::from_str(&request.method))
            ...
            let mut body =
                serde_json::from_slice::<HashMap<String, serde_json::Value>>(&request.body)
            ...
            if let Some(url_path) = body.get("__url_path") {
                ...
                let parsed_url = url::Url::parse(url_path).expect(
                    "expected the url_path to be a valid url path, check the __url_path key",
                );
                let query_pairs: HashMap<_, _> = parsed_url.query_pairs().into_owned().collect();

                let query = js_sys::Reflect::get(&req, &JsValue::from_str("query"))
                    .expect("expected req to have a query property");
            ...
            let path = match url::form_urlencoded::parse(path.as_bytes()).next() {
                ...
                None => {
                    js_sys::Reflect::set(&res, &"statusCode".into(), &JsValue::from_f64(500.0))
                        .expect("expected res to be a mutable object");
                    js_sys::Reflect::set(
                        &res,
                        &"statusMessage".into(),
                        &JsValue::from_str("Internal Server Error"),
                    )
                    .expect("expected res to be a mutable object");
            ...
            let path_ = dir
                .as_string()
                .expect("expected the dir to be a string; qed")
                + &path;

            let exists_sync = js_sys::Reflect::get(&fs, &JsValue::from_str("existsSync"))
                .expect("expected fs to have an existsSync method");
            ...
            if !exists {
                js_sys::Reflect::set(&res, &"statusCode".into(), &JsValue::from_f64(404.0))
                    .expect("expected res to be a mutable object");
                js_sys::Reflect::set(
                    &res,
                    &"statusMessage".into(),
                    &JsValue::from_str("Not Found"),
                )
                .expect("expected res to be a mutable object");
                ...
                let end = js_sys::Function::from(end);
                    ...
                    &JsValue::NULL,
                    &JsValue::from_str(&format!("Cannot GET {path}")),
                )
                .expect("expected end to be a function");
            ...
            // read the file
            let read_file_sync = js_sys::Reflect::get(&fs, &JsValue::from_str("readFileSync"))
                .expect("expected fs to have a readFileSync method");
            ...
            // encrypt the file
            let encrypted = match symmetric_key.symmetric_encrypt(
                &serde_json::to_vec(&js_res)
                    .expect("expected the response to be serializable to a valid json object; qed"),
            ) {
                Ok(val) => val,
                Err(err) => {
            ...
            js_sys::Reflect::set(
                &res,
                &"statusCode".into(),
                &JsValue::from_f64(js_res.status as f64),
            )
            .expect("expected res to be a mutable object");
            js_sys::Reflect::set(
                &res,
                &"statusMessage".into(),
                &JsValue::from_str(&js_res.status_text),
            )
            .expect("expected res to be a mutable object");
            ...
            js_sys::Reflect::set(
                &data,
                &"data".into(),
                &JsValue::from_str(&base64_enc_dec.encode(&encrypted)),
            )
            .expect("expected data to be a mutable object");

            end.call1(&JsValue::NULL, &JsValue::from(data))
                .expect("expected end to be a function");
        ...
        if let Err(err) = add_event_listener.call1(&JsValue::from_str("data"), on_data) {
            console_error(
                &err.as_string().unwrap_or(
                    " Error: Failed to add event listener to request object.".to_string(),
                ),
            );
            return JsValue::NULL;
        }
